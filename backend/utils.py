...
def send_email(to_email: str, subject: str, html_body: str):
    ...
    def _send():
        ...
        if not all([smtp_host, smtp_port, smtp_user, smtp_pass]):
            logger.error("SMTP Configuration missing in environment. Cannot send email.")
            return

        try:
            msg = MIMEMultipart("alternative")
            msg["Subject"] = subject
            msg["From"] = f"KairavCard <{smtp_user}>"
            msg["To"] = to_email
    ...
    # Run in background thread to avoid blocking Flask request
